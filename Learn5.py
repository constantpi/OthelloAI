...
if(__name__ == '__main__' ):
    ...
    for i in range(len(y_data)):
        one_data=copy.copy(count_data[i])
        for j in range(4):
            one_data+=kado_data[j][i]
        for j in range(6):
            one_data+=hen_data[j][i]
        allData.append(one_data)


    print(count_data[0],kado_data[0][0],hen_data[0][0])

    trainData=np.array(allData[:trainNum])
    testData=np.array(allData[trainNum:])
    ...
    #     # x=Activation("relu")(x)
    #     x=Dense(1,name="Dense2"+extractInputName[i])(x)
    #     # x=BatchNormalization()(x)
    #     x=Activation("relu")(x)
    ...
    # x=Dense(100)(combined)

    input=Input(shape=(356),name="Input")
    ...
    # x=Dense(320,activation="relu")(input)
    # x=Dense(100,activation="relu")(x)
    # x=Dense(100,activation="relu")(x)
    # x=Dense(100,activation="relu")(x)
    # x=Dense(100,activation="relu")(x)
    ...
    CNN_model=tensorflow.keras.Model(inputs=input,outputs=output)
    ...
    # packsize=40000
    print()
    for i in range(trainData.shape[0]//packsize):
        trainPack=[]
